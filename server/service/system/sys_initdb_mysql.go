...
// WriteConfig writes MySQL configuration back
...
// EnsureDB creates and initializes the MySQL database
func (h MysqlInitHandler) EnsureDB(ctx context.Context, conf *request.InitDB) (next context.Context, err error) {
	...
	if c.Dbname == "" {
		return ctx, nil
	} // If there is no database name, exit the initialization data
	...
	if db, err = gorm.Open(mysql.New(mysql.Config{
		...
		SkipInitializeWithVersion: true,    // Automatically configure based on the version
